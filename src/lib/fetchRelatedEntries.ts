...
import type { ConditionEntry, ResourceLink, TreatmentEntry } from "./types";
...
export async function fetchRelatedEntries(
    ...
    entries.map(async (entry) => {
      if (entry?.sys?.type === "ResourceLink") {
        try {
          ...
          console.error(
            `Failed to fetch entry with URN ${entry.sys.urn}:`,
            error,
          );
          return null;
        }
  ...
  return results
    .filter(
      (
        result,
      ): result is PromiseFulfilledResult<ConditionEntry | TreatmentEntry> =>
